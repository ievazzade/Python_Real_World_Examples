...
class BinaryTree:
    ...
    def find_in_bsg(self, node_data):
        return self.find_in_bst_rec(self.root, node_data)
    
    def populate_parent_rec(self, node, parent):
        if node:
            node.parent = parent
            self.populate_parent_rec(node.left, node)
            self.populate_parent_rec(node.right, node)
    
    def populate_parent(self):
        self.populate_parent_rec(self.root, None)
